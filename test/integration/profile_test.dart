...
void main() {
  ...
  test('shared prefs to GPT test', () async {
    SharedPreferences.setMockInitialValues({
      ...
      ageKey: '37',
      storyKey:
          'Ive been a project manager for a mid-sized tech company for the past five years.',
    });
        ...
        "messages": [
          {
            "role": "user",
            "content":
                "My name is $fullName, I am $age years old, and here's a little about me: $story. Actual question/action implemented here"
          }
        ]
      }),
    );
    ...
    if (response.statusCode == 429) {
      expect(jsonResponse['error']['code'], 'insufficient_quota',
          reason: 'Quota exceeded, but this is expected for this test case.');
    } else {
      // For all other responses, expect a successful status code
      expect(response.statusCode, 200,
          reason: 'Failed to get response from ChatGPT');
    }
  });
}
